...
func assignOrders(stateToAssign <-chan slave.WorldView, orderAssignments chan<- [config.N_ELEVATORS][config.N_FLOORS][config.N_BUTTONS]bool) {
	for {
		select {
...
func transformOutput(outputJsonFormat []byte, state slave.WorldView) [config.N_ELEVATORS][config.N_FLOORS][config.N_BUTTONS]bool {
	...
	tempOutput := new(map[string][slave.N_FLOORS][2]bool)
